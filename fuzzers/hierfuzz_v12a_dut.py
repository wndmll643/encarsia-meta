"""HierFuzz v12a coverage variant.

Uses hierfuzz_instrument_v12a Yosys pass for coverage instrumentation.
...
class HierFuzzV12aDUT():
    def __init__(self, host: Host, bug: Bug):
        self.directory = os.path.join(bug.directory, "hierfuzz_v12a")
    ...
    def create_dut(self):
        ...
        if not os.path.exists(host_rtlil):
            ...
            print(f"Warning: skipping hierfuzz_v12a for bug {self.bug.name} (no host.rtlil)")
            return self

        self.module = os.path.join(self.directory, "host.v")
        if not os.path.exists(self.module):
            subprocess.run(
                [defines.YOSYS_PATH, '-c', self.host.hierfuzz_v12a_export_script],
                check=True,
                cwd=self.directory,
    ...
    def create_reference(self):
        ...
        if not os.path.exists(self.reference):
            subprocess.run(
                [defines.YOSYS_PATH, '-c', self.host.hierfuzz_v12a_ref_export],
                check=True,
                cwd=self.directory,
                stdout=subprocess.DEVNULL
            )
