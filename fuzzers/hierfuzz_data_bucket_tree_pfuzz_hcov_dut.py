"""HierFuzz data_bucket_tree coverage + ProcessorFuzz mutator + hierCov gate.

Same as data_bucket_pfuzz_hcov but with data_bucket_tree instrumentation
(data inputs + bucket-XOR + lossless tree-sum aggregation via
...
class HierFuzzDataBucketTreePfuzzHcovDUT():
    def __init__(self, host: Host, bug: Bug):
        self.directory = os.path.join(bug.directory, "hierfuzz_data_bucket_tree_pfuzz_hcov")
    ...
    def create_dut(self):
        ...
        if not os.path.exists(host_rtlil):
            ...
            print(f"Warning: skipping hierfuzz_data_bucket_tree_pfuzz_hcov for bug {self.bug.name} (no host.rtlil)")
            return self
