...
class HierFuzzDataBucketDUT():
    def __init__(self, host: Host, bug: Bug):
        self.directory = os.path.join(bug.directory, "hierfuzz_data_bucket")
    ...
    def create_dut(self):
        # Use plain host.rtlil — Yosys hierfuzz_instrument_data_bucket pass adds coverage
        host_rtlil = os.path.join(self.bug.directory, "host.rtlil")
        if not os.path.exists(host_rtlil):
            self.compile_failed = True
            print(f"Warning: skipping hierfuzz_data_bucket for bug {self.bug.name} (no host.rtlil)")
            return self
