"""HierFuzz ctrl_bucket_tree coverage + ProcessorFuzz mutator + hierCov gate.

Same as data_bucket_pfuzz_hcov but with ctrl_bucket_tree instrumentation
(control inputs + bucket-XOR + lossless tree-sum aggregation via
io_hierCovSumTotal). HIER_COV_TOTAL=1 is set so the patched cocotb host
reads the tree-summed port instead of io_hierCovSum.
"""
...
class HierFuzzCtrlBucketTreePfuzzHcovDUT():
    def __init__(self, host: Host, bug: Bug):
        self.directory = os.path.join(bug.directory, "hierfuzz_ctrl_bucket_tree_pfuzz_hcov")
        ...
        # Tell the patched RTLSim/host.py to read io_hierCovSumTotal (lossless
        # tree-sum) so ProcessorFuzz's `coverage` matches the new corpus-gate.
    ...
    def create_dut(self):
        ...
        if not os.path.exists(host_rtlil):
            ...
            print(f"Warning: skipping hierfuzz_ctrl_bucket_tree_pfuzz_hcov for bug {self.bug.name} (no host.rtlil)")
            return self
