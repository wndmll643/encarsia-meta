...
Uses hierfuzz_instrument_data_bucket Yosys pass for coverage instrumentation,
...
class HierFuzzDataBucketPfuzzDUT():
    def __init__(self, host: Host, bug: Bug):
        self.directory = os.path.join(bug.directory, "hierfuzz_data_bucket_pfuzz")
    ...
    def create_dut(self):
        ...
        if not os.path.exists(host_rtlil):
            ...
            print(f"Warning: skipping hierfuzz_data_bucket_pfuzz for bug {self.bug.name} (no host.rtlil)")
            return self
        ...
        if not os.path.exists(self.dut_path):
            with open(self.dut_path, 'w') as dut_file:
                # Use ProcessorFuzz receptor (same port interface as DifuzzRTL)
