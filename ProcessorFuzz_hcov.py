"""ProcessorFuzz entry point that drives the hcov-gated Run() coroutine.

Identical to /encarsia-processorfuzz/Fuzzer/ProcessorFuzz.py except that
`Run` is imported from `Fuzzer_hcov` instead of `Fuzzer`. The new hcov-gated
DUT classes select this module via `MODULE=ProcessorFuzz_hcov` in their
make invocations.

Install path inside container: /encarsia-processorfuzz/Fuzzer/ProcessorFuzz_hcov.py
"""
...
# hcov fork: pull Run from the forked Fuzzer_hcov module instead of Fuzzer.
from Fuzzer_hcov import Run
...
def wait_check(proc_pid_arr: list, manager: procManager):
    ...
    if exit_code != 0:
        print('[ProcessorFuzz/hcov] Something bad happens! exit_code: {}'.format(exit_code))
    ...
    if state != proc_state.NORMAL:
        print('[ProcessorFuzz/hcov] Child {} is in abnormal state {}!'.format(idx, proc_state.tpe[state]))
        for pid in proc_pid_arr:
            if pid != end_pid:
                os.kill(pid, signal.SIGKILL)
        exit(-1)
...
if not multicore:
    ...
    print('[ProcessorFuzz/hcov] Muticore Fuzzing, Number of batches -- {}'.format(num_batch))

    parent = True
    num_running = 0
    proc_num = 0
    proc_pid_arr = [ 0 for i in range(multicore) ]
    for i in range(num_batch * multicore):
        proc_num = proc_pid_arr.index(0)
        print('[ProcessorFuzz/hcov] Multicore Fuzzing Batch [{}], Thread [{}]'.format(i, proc_num))
    ...
    if not parent:
        ...
        print('[ProcessorFuzz/hcov] Stop Multicore Fuzzing')
